...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Soldadores</div>
                    <div class="card=body">
                        <div class="container mt-3">
                            ...
                            @else
                            <br>
                                <h4 align="center">No hay soldadores disponibles en el sistema</h4>
                            <br>
                            @endif
                            <br>
                        </div>
            ...
            <a class="btn btn-primary btn-lg" role="button" href="{{url('/soldadura/soldaduraControl', [$idProducto])}}"><b>Volver</b></a>
        </div>
    </div>
</div>
<script type="text/javascript">
    ...
    function asignarTrabajo(idProducto, idTrabajador)
    {
        ...
        $.ajax({
            ...
            type: "POST",
            data: {DATA:json_data},
            url: "{{url('/productoControl/addWorkerSoldador')}}",
            success: function(response){
                if(response == 1)
                    window.location.href = "{{url('producto/asignarTrabajoSoldador', [$idProducto])}}";
                else
                  if(response == 2)
                    showMensajeSwal(MSG_ERROR, BTN_ERROR, COLOR_ERROR,'El trabajador ya está asignado al producto');
                  else
                      showMensajeSwal(MSG_ERROR, BTN_ERROR, COLOR_ERROR,'Error al asignar al soldador');
            }
        });
    }
</script>
